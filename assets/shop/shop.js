...
function productCard(p){const b=brandMap.get(p.brand_id);const price=money(p);const sold=p.status==='soldout';const discount=isDiscounted(p);return `<a class="product-card" href="/item.html?id=${encodeURIComponent(p.id)}"><div class="product-image"${p.motion_image?` data-motion-image="${safe(p.motion_image)}"`:""}><img src="${safe(p.image)}" alt="${esc(p.name)}" loading="lazy" decoding="async" referrerpolicy="no-referrer">${sold?'<span class="badge">품절</span>':p.kind==='rental'?'<span class="badge rental">렌탈</span>':discount?'<span class="badge sale-badge">SALE</span>':isPromotion(p)?'<span class="badge sale-badge">PROMOTION</span>':''}</div><div class="product-brand">${esc(b?.name||'')}</div><h3 class="product-name">${esc(p.name)}</h3><p class="product-price">${discount?`<span class="price-before">${won(p.price)}원</span>`:''}${p.kind==='rental'?globalThis.NadaunRental.priceHTML(p):price===null?'가격 문의':`${won(price)}<small>원</small>`}</p>${p.kind==='rental'?'<p class="card-note">일정 확인 후 예약</p>':p.status==='inquiry'?'<p class="card-note">구매·입고 상담</p>':''}</a>`}
