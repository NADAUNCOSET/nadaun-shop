# -*- coding: utf-8 -*-
"""godo.py ALL 수퍼바이저 — 소스 3곳 순차 실행 (cafe24_supervisor 패턴, 2026-07-17).
   출력 10분 무응답이면 kill 후 체크포인트(data/_<site>_done.json)에서 재시작.
   완료 신호('완료 브랜드별 누계') 보이면 다음 소스로. 실행: python -u godo_supervisor.py"""
import subprocess, sys, io, time, threading, os
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8")

HERE = os.path.dirname(os.path.abspath(__file__))
SCRIPT = os.path.join(HERE, "godo.py")
STALL_SEC = 600
MAX_RESTARTS = 40
ORDER = ["redsun","calla","lmount"]
