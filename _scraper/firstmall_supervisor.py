# -*- coding: utf-8 -*-
"""firstmall.py ALL 수퍼바이저 — avx·aurora·sajin 순차 실행 (cafe24_supervisor 패턴, 2026-07-17).
   출력 10분 무응답이면 kill 후 체크포인트(data/_<site>_done.json)에서 재시작.
   실행: python -u firstmall_supervisor.py"""
import subprocess, sys, io, time, threading, os
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8")

HERE = os.path.dirname(os.path.abspath(__file__))
SCRIPT = os.path.join(HERE, "firstmall.py")
STALL_SEC = 600
MAX_RESTARTS = 40
ORDER = ["avx","aurora","sajin","green"]
