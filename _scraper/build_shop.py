...
/* 원본 브랜드몰과 동일한 2단 카테고리 메뉴 (2026-08-05) */
.l1{display:flex;flex-wrap:wrap;gap:4px;padding:9px 0 10px;border-bottom:1px solid var(--line)}
.l1 button{border:0;background:transparent;border-radius:8px;padding:8px 13px;font-size:13px;font-weight:600;
  cursor:pointer;color:var(--ink);font-family:inherit;transition:.25s var(--ease)}
.l1 button:hover{background:#f2efe9}
.l1 button.on{background:var(--ink);color:#fff}
.l1 button .c{opacity:.5;margin-left:5px;font-weight:400;font-size:11.5px}
.l2{display:flex;flex-wrap:wrap;gap:6px;align-items:center;padding:11px 0 3px}
.l2h{font-size:12.5px;color:#6b6660;margin-right:6px}
.l2h b{color:var(--ink)}
.l2 button{border:1px solid var(--line);background:#fff;border-radius:999px;padding:6px 13px;font-size:12.5px;
  font-weight:600;cursor:pointer;font-family:inherit;transition:.25s var(--ease)}
.l2 button:hover{border-color:var(--gold)}
.l2 button.on{background:var(--ink);color:#fff;border-color:var(--ink)}
.l2 button .c{opacity:.5;margin-left:4px;font-weight:400}
...
let mode=MODE0, curCat='all', curBrand=null, curSub='전체', curTop='전체', shown=PAGE;
...
// ── 제품별 ──
// 대분류를 고르면 그 하위(대 > 중)까지 포함 — KPP 브랜드몰 동작과 동일
function subMatch(d){
  if(curSub==='전체') return true;
  return d.sub===curSub || (d.sub||'').startsWith(curSub+' > ');
}
function pool(){return DATA.filter(d=>(curCat==='all'||d.c===curCat)&&(!curBrand||d.s===curBrand)&&subMatch(d))}
// ★ 원본 브랜드몰과 동일한 2단 카테고리 메뉴 (2026-08-05 대표 지정)
//   1차 = 대분류 가로 바 (카메라 케이지·폰리그·핸들 …)
//   2차 = 선택한 대분류의 하위 (케이지·비디오 촬영 키트·필터 …)
function renderSubs(){
  const bar=$('subbar');
  ...
  const tree={}, topCnt={};
  p.forEach(d=>{
    const seg=(d.sub||'기타').split(' > ');
    const a=seg[0], b=seg.slice(1).join(' > ');
  ...
  // 2차 (선택된 대분류에 하위가 있을 때만)
  if(curTop!=='전체'){
    const subs=Object.keys(tree[curTop]||{}).sort((x,y)=>tree[curTop][y]-tree[curTop][x]);
    h+=`<div class="l2"><span class="l2h">카테고리 : <b>${esc(curTop)}</b> (총 ${topCnt[curTop]||0}개)</span>`+
      `<button data-j="${esc(curTop)}" class="${curSub===curTop?'on':''}">${esc(curTop)}</button>`+
      subs.map(b=>`<button data-j="${esc(curTop+' > '+b)}" class="${curSub===curTop+' > '+b?'on':''}">${esc(b)}<span class="c">${tree[curTop][b]}</span></button>`).join('')+`</div>`;
  }
  $('subs').innerHTML=h;
  bar.classList.add('open');
  $('subs').querySelectorAll('.l1 button').forEach(b=>b.onclick=()=>{
    curTop=b.dataset.top; curSub=(curTop==='전체'?'전체':curTop); shown=PAGE; render();
  });
  $('subs').querySelectorAll('.l2 button').forEach(b=>b.onclick=()=>{curSub=b.dataset.j;shown=PAGE;render()});
}
function card(d){
