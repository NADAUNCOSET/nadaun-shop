...
def install():
    ...
    definition={'Label':LABEL,'ProgramArguments':[sys.executable,str(ROOT/'_scraper/shop_sync.py'),'--scheduled'],
      'WorkingDirectory':str(ROOT),'StartCalendarInterval':{'Hour':8,'Minute':0},
      'RunAtLoad':True,'ProcessType':'Background',
      'StandardOutPath':str(STATE/'worker.stdout.log'),'StandardErrorPath':str(STATE/'worker.stderr.log'),
    ...
    save_json(STATE/'schedule.json',{'label':LABEL,'schedule':'매일 08:00 Asia/Seoul','installed_at':stamp(),'host':os.uname().nodename,'source':'_scraper/shop_sync.py','requires':'Mac on, NAS mounted, network available'})
